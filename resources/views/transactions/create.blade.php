<x-app-layout>
                    ...
                    <form method="POST" action="{{ route('transactions.store') }}">
                        ...
                        <div class="mt-4">
                            ...
                            <select id="payment_method" name="payment_method" class="block mt-1 w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-md shadow-sm focus:ring focus:ring-opacity-50" required>
                                ...
                                <option value="debit" {{ old('payment_method') == 'debit' ? 'selected' : '' }}>Debit Card</option>
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="total_price" class="block mt-1 w-full" type="text" name="total_price" :value="old('total_price', 0)" readonly />
    ...
    <!-- Template for transaction item row -->
    <template id="item-template">
        <div class="item-row flex gap-4 items-end">
            <div class="flex-1">
                <x-input-label for="menu_id" :value="__('Menu Item')" />
                <select name="items[][menu_id]" class="block mt-1 w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-md shadow-sm focus:ring focus:ring-opacity-50 menu-select" required>
            ...
            <div class="w-24">
                ...
                <x-text-input type="number" name="items[][quantity]" min="1" value="1" class="quantity-input block mt-1 w-full" required />
            </div>
            <div class="w-32">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function addItemRow() {
                ...
                // Add remove event to the new item
                ...
                // Calculate initial subtotal for new item
                const menuSelect = itemsContainer.lastElementChild.querySelector('.menu-select');
                const quantityInput = itemsContainer.lastElementChild.querySelector('.quantity-input');
                const subtotalInput = itemsContainer.lastElementChild.querySelector('.subtotal-input');
