        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-sm">
                <div class="mx-auto py-4 px-4 sm:px-6 lg:px-8 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('menus.update', $menu->menu_id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <x-text-input id="menu_name" class="block mt-1 w-full" type="text"
                                name="menu_name" :value="$menu->menu_name ?? old('menu_name')" required autofocus />
                            ...
                            <select id="category_id" name="category_id" required
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->category_id }}"
                                        {{ ($menu->category_id == $category->category_id || old('category_id') == $category->category_id) ? 'selected' : '' }}>
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="price" class="block mt-1 w-full" type="number" step="0.01"
                                name="price" :value="$menu->price ?? old('price')" required />
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="description" name="description" rows="3"
                                class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                            >{{ $menu->description ?? old('description') }}</textarea>
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="stock" class="block mt-1 w-full" type="number"
                                name="stock" :value="$menu->stock ?? old('stock', 0)" required />
                            ...
                            <div class="mt-2">
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="is_available" value="1"
                                        {{ ($menu->is_available || old('is_available', true)) ? 'checked' : '' }}
